...
<body class="bg-gray-900 text-gray-100">
    <a href="#main-content"
        class="sr-only focus:not-sr-only focus:absolute focus:top-0 focus:left-0 focus:z-50 focus:bg-blue-600 focus:text-white focus:p-2">Skip
        to main content</a>

    <header role="banner"
        class="mt-8 mb-8 text-center bg-gradient-to-r from-blue-600 to-indigo-600 py-12 px-4 rounded-lg shadow-xl">
    ...
    <div class="max-w-7xl mx-auto px-4 py-10">
                    ...
                    <form method="post" novalidate>
                        <fieldset class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    <label for="sip" class="block text-sm font-medium text-gray-300 mb-2">Monthly SIP
                                        Investment</label>
                                    <input type="number" step="0.01" id="sip" name="sip"
                                        ...
                                        required min="1" aria-required="true" aria-label="Monthly SIP Investment amount"
                                        value="<?= htmlspecialchars((string) $sip) ?>">
                                </div>
                                <div>
                                    <label for="years" class="block text-sm font-medium text-gray-300 mb-2">Years of
                                        Investment</label>
                                    <input type="number" id="years" name="years"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="1" aria-required="true" aria-label="Years of Investment"
                                        value="<?= htmlspecialchars((string) $years) ?>">
                                </div>
                                <div>
                                    <label for="rate" class="block text-sm font-medium text-gray-300 mb-2">Annual
                                        Interest Rate (% p.a.)</label>
                                    <input type="number" step="0.01" id="rate" name="rate"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="0" aria-required="true"
                                        aria-label="Annual Interest Rate percent per annum"
                                        value="<?= htmlspecialchars((string) $rate) ?>">
                                </div>
                                <div>
                                    <label for="stepup" class="block text-sm font-medium text-gray-300 mb-2">Annual SIP
                                        Increase (%)</label>
                                    <input type="number" step="0.01" id="stepup" name="stepup"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="0" aria-required="true" aria-label="Annual SIP Increase percent"
                        ...
                        <fieldset class="mb-6">
                            ...
                            <p class="text-sm text-gray-400 mb-4">Note: SWP automatically starts in the year immediately
                                following your SIP period. Monthly withdrawals are capped to available funds.</p>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    <label for="swp_withdrawal"
                                        class="block text-sm font-medium text-gray-300 mb-2">Monthly SWP
                                        Withdrawal</label>
                                    <input type="number" step="0.01" id="swp_withdrawal" name="swp_withdrawal"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="0" aria-required="true" aria-label="Monthly SWP Withdrawal amount"
                                        value="<?= htmlspecialchars((string) $swp_withdrawal) ?>">
                                </div>
                                <div>
                                    <label for="swp_stepup" class="block text-sm font-medium text-gray-300 mb-2">Annual
                                        SWP Increase (%)</label>
                                    <input type="number" step="0.01" id="swp_stepup" name="swp_stepup"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="0" aria-required="true" aria-label="Annual SWP Increase percent"
                                        value="<?= htmlspecialchars((string) $swp_stepup) ?>">
                                </div>
                                <div>
                                    <label for="swp_years" class="block text-sm font-medium text-gray-300 mb-2">Number
                                        of SWP Years</label>
                                    <input type="number" id="swp_years" name="swp_years"
                                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-400"
                                        required min="1" aria-required="true" aria-label="Number of SWP Years"
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button type="submit" name="action" value="calculate"
                                aria-label="Calculate investment results"
                            ...
                            <button type="submit" name="action" value="download_csv"
                                ...
                                class="px-6 py-2 bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400 rounded font-medium transition-colors">Download
                                CSV Report</button>
            ...
            <section class="card-gradient rounded-lg mb-6 p-6" aria-label="Investment growth visualization">
                <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-400">Interactive chart: Use mouse scroll to zoom, pinch on touch to
                            zoom, drag to select zoom area. Hold Alt and drag to pan.</p>
                    </div>
                ...
                <div class="relative" style="height: 360px;" role="img"
                    aria-label="Line chart showing corpus, cumulative investment, and SWP withdrawals over years">
                            ...
                            <thead class="table-header-gradient border-b border-gray-700">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left font-semibold"
                                        ...
                                        Contribution</th>
                                    <th class="px-4 py-3 text-left font-semibold"
                                        title="Total SIP contributed cumulatively up to this year">Total SIP Invested to
                                        Date
                                    </th>
                                    <th class="px-4 py-3 text-left font-semibold"
                                    ...
                                    <th class="px-4 py-3 text-left font-semibold"
                                        title="Total SWP withdrawn cumulatively up to this year">Total SWP Withdrawals to
                                        Date
                                    </th>
                                    <th class="px-4 py-3 text-left font-semibold"
        ...
        <footer role="contentinfo"
            class="bg-gray-900 border-t border-gray-800 mt-12 py-6 px-4 text-center text-sm text-gray-400">
            <p>
                <span class="text-red-500 font-semibold">Disclaimer:</span> This calculator is for illustrative purposes
                only. It does not constitute financial, tax, or investment advice. Please consult a qualified financial
