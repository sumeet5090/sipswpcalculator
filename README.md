...
- **Modular Data Repositories (`InsightRepository`, `GlossaryRepository`, `FaqRepository`, `MetaManager`):** Repositories execute safe file I/O checks (`file_get_contents` return validation and `json_last_error()` checks) and adhere strictly to SRP. Date fallbacks across `BlogRepository` and `ContentManager` are unified under `DateConstants::CONTENT_FALLBACK_DATE`.
...
- **Strict TypeScript & Full Parity:** `MathEngine.ts` precision is fully aligned with `InvestmentCalculator.php`. All event topics, timers, chart configurations, goal modes (`export type GoalMode = 'grow' | 'target' | 'target_corpus'`), and strategy contracts (`abstract class CalculatorStrategy`) are strongly typed without `any` bypasses.
