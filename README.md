...
- **Fintech-Grade Mobile UI/UX & Cognitive Ergonomics Architecture:** Drastic mobile interface redesign delivering a clean, frictionless experience adhering to top-tier fintech standards (Groww, Zerodha, Monarch Money).
  - *Progressive Disclosure & Zero Scroll Fatigue:* Lengthy editorial paragraphs and author bios are tucked into accessible `<details class="fintech-accordion">` disclosures on mobile viewports while preserving full desktop rendering (`calculator-guide.twig`, `home.twig`). The `#quick-answer` snippet is transformed into a compact, expandable summary pill preserving AMFI/SEBI speakable schema selectors. Sub-calculator guides (`content_html`) and FAQs are wrapped in light fintech accordions on mobile, dropping mobile page height by >75% (from 27,000+ px to ~2,200 px).
  - *Thumb-Friendly Touch Targets & Tactile Sliders:* Input steppers (`[-]`/`[+]`) provide generous 48px touch targets with active scale feedback. Range slider thumbs feature 26px tactile round handles (`resources/css/input.css`) with emerald focus rings and grab/grabbing cursors. Preset pills are organized into horizontally scrollable chips (`no-scrollbar`) to eliminate multi-line grid clutter.
  - *Real-Time Mobile Glance Cockpit (`#mobile-action-dock`):* Fixed bottom dock featuring live calculation telemetry (`dock-glance-corpus`, `dock-glance-invested`, `dock-glance-gains`) that updates instantaneously during slider adjustments, accompanied by a single-tap `Summary ↑` jump button and mode toggles. Active modes (`SIP` vs `SWP`) are dynamically synchronized based on route and calculator type, with route fallback navigation when standalone calculators lack in-DOM tabs.
  - *Single Bottom Dock Ergonomics (Zero Overlap Architecture):* Strictly eliminates multi-bar floating clutter on mobile devices by making `#floating-discovery-hud` desktop-only (`hidden sm:block`) and integrating quick section jump anchors (`⚡ Calculator`, `📊 Breakdown`, `📐 Formulas`, `❓ FAQs`) directly into the native `#mobile-actions-sheet` drawer (triggered via `•••`). The scroll-to-top FAB (`#mobile-scroll-top-fab`) is anchored at `bottom-[140px]`, floating cleanly above the dock without visual collision.
  - *Visual Proportional Milestone Breakdown (`ResultsController.ts`):* Mobile milestone cards (`#mobile-breakdown-cards`) feature dual-colored proportional progress bars directly illustrating the ratio of Principal Invested vs Compounding Gains, accompanied by multiplier badges and clean typography.
  - *Collapsible Editorial Knowledge Base (`home-guide-content.twig` & `calculator-guide.twig`):* Dense multi-section financial guides are restructured into an animated accordion stack with smooth chevron rotation, allowing users to browse topics without cognitive overload.
- **Calculator Input Mode Isolation & SWP Calculation Integrity:** `CalculatorApp.getInputs()` strictly isolates form parameter collection by calculator mode (`swp`, `lumpsum`, `sip`). On single-mode SWP calculators, accumulation parameters (`sip`, `years`, `rate`, `stepup`) are explicitly set to zero rather than falling through to validator minimum clamps (`500`, `1`), guaranteeing 100% calculation parity with `InvestmentInputs::fromSwpRequest()`, preventing artificial 1-year accumulation phases, and eliminating false early depletion warnings. Summary controllers and insight radars dynamically select the effective return rate (`swp_rate` in SWP mode) for Rule of 72 doubling calculations and compounding multiplier telemetry.
...
- **Institutional-Grade Canvas & Visualizer Hardening (`ChartManager.ts`, `ChartScrubbingController.ts`, `chart-visualization.twig`):** Implements a high-DPI subpixel canvas scaling engine with a safe 2.5x DPR ceiling, quantized linear gradient bucketing (30px quantizing intervals), and RAF render queue throttling to guarantee 60fps interaction during rapid slider adjustments. Features:
  1. **4-Tier Streamlined Command & Inspection Architecture:** (Tier 1) Authoritative Headline identity dock with AMFI-aligned certification pill, real-time projected gross corpus (`#chart-header-gross`), and net gains telemetry (`#chart-header-gain`); (Tier 2) Real-Time Timeline Inspection HUD dock (`#chart-inspection-hud`) displaying dynamic year, invested, compounding gains, and gross total with live pulsating indicator dot; (Tier 3) High-DPI interactive canvas container with accessible assistive table ledger; and (Tier 4) Bottom opportunity cost benchmark chips (None, Nifty 50 @ 12%, Gold @ 9%, Bank FD @ 6.5%) and consolidated analytical projection lenses (`§112A LTCG`, `± Corridor`, `Wealth Map`).
  2. **Deterministic Harmonic Year Ticks Generator (`computeHarmonicYearTicks`):** Replaces decimation dropouts and irregular tick skipping (such as `Y1`, `Y5`, `Y15` skipping `Y10`) with an exact, harmonic milestone cadence (`Yr 1`, `Yr 5`, `Yr 10`, `Yr 15`, `Yr 20`...) with `autoSkip: false`, ensuring every decade and landmark year is visibly anchored.
  3. **Canvas State Isolation & Zero-Bleed Guardrails:** All custom in-canvas plugins (`croreMilestoneLinePlugin`, `compoundingIgnitionPlugin`, `fdAlphaDeltaPlugin`, `splineMilestonesPlugin`, `donutCenterTextPlugin`, `crosshairPlugin`) strictly enforce canvas state preservation with `try ... finally { ctx.restore(); }` guards, completely preventing dashed line styles or stroke configurations from bleeding into native dataset curves.
  4. **Collision-Free Milestone Pins & Compounding Ignition:** The ₹1 Crore Milestone is rendered as a clean, right-anchored milestone pin badge with edge clearance, eliminating horizontal line slicing through curves and text. The Compounding Ignition zone is rendered as an ambient soft light aurora gradient (`rgba(16, 185, 129, 0.08)`) with a top hairline beacon, eliminating floating dashed box collisions.
  5. **Time-Travelling Donut Morpher & SWP Depletion Sentinel:** Displays dynamic ROI multipliers (`donutCenterTextPlugin`) and turns into a high-visibility Rose-700 bankruptcy sentinel on depleted portfolios.
  6. **Decoupled Mobile Tactile Range Scrubber (`#mobile-chart-scrubber`):** Haptic pulse feedback with active timeline bubble (`#scrubber-active-indicator`), resolving thumb occlusion and eliminating mobile canvas scroll-locking.
...
- **Mobile Touch Ergonomics & Viewport Resiliency Architecture (`KeyboardViewportController.ts`, `ChartScrubbingController.ts`, `MobileErgonomicDeckController.ts`, `BottomSheetGestureController.ts`, `WebHapticEngine.ts`, `base.twig`):** Establishes an uncompromising mobile-first experience across compact viewports (320px–430px) adhering to Apple HIG (44pt) and Material Design 3 (48dp):
  1. **Single Source of Truth Virtual Viewport Sync:** Replaces competing inline scripts with unified visual viewport management in `KeyboardViewportController.ts`, animating keyboard offsets via GPU-accelerated CSS custom property `--keyboard-offset` and auto-scrolling inputs to center view with zero header clipping.
  2. **iOS Numeric Pad Dismissal Accessory Bar:** Pinned micro-capsule directly above software keyboards featuring live projected corpus, input field traversal steppers (Prev/Next), and an explicit `Done` button to cleanly blur inputs.
  3. **Directional Slope-Locked Canvas Touch Scrubbing:** Restores direct chart canvas interactivity (`touch-action: pan-y`) while eliminating scroll traps through mathematical slope disambiguation ($\Delta x / \Delta y > 1.2$), syncing seamlessly with haptic feedback ticks.
  4. **3-Zone Ergonomic Bottom Command Hub & Action Sheet:** Refactors congested action docks into an ergonomic 3-zone command bar (PDF Plan Hero, WhatsApp Share, and "•••" drawer trigger opening a native-styled bottom sheet for secondary utilities).
  5. **Physics-Driven Bottom Sheet Touch Drag-to-Dismiss (`BottomSheetGestureController.ts`):** Implements smooth touch gestures on mobile dialogs (`#mobile-actions-sheet`, `#qr-share-modal`, `#sebi-benchmark-modal`) with directional slope locking, GPU-accelerated `translateY` transforms, threshold flick velocity detection (>80px or >0.4px/ms), and mechanical haptic tick feedback.
  6. **Mobile Share Flow Polymorphism (`qr-share-modal.twig`, `ShareController.ts`):** Elevates the Native OS Share API (`navigator.share`), WhatsApp Proposal, and direct image download on mobile devices while wrapping the 160px QR canvas in a collapsible disclosure accordion, eliminating the phone-scanning paradox.
  7. **Horizontal Snap Rails for Scenario Comparison (`city-fire-benchmark.twig`, `stress-test-simulator.twig`):** Replaces rigid 2-column mobile grids with fluid horizontal snap rails (`snap-x snap-mandatory`), resolving Kolkata's 5th-card grid asymmetry and eliminating text wrapping on narrow 320px–360px viewports.
  8. **Visual Multi-Asset Proportional Micro-Bars (`asset-rebalancing.twig`):** Embeds instant visual distribution bars inside portfolio allocation presets (80/20, 70/30, 60/25/15, 50/35/15) for immediate glanceability.
  9. **Mobile Floating Section Discovery HUD (`floating-discovery-hud.twig`):** Thumb-reachable persistent navigation pill positioned at `bottom-[74px]` providing 1-tap jumping between Calculator, Breakdown Table, Math Formulas, and FAQs without vertical thumb fatigue.
  10. **Contained Mobile Navigation Menu (`header.twig`):** Encapsulates the hamburger menu within `max-h-[calc(100vh-4.5rem)] overflow-y-auto overscroll-contain shadow-modal`, completely eliminating page pushdown.
  11. **Asymmetric Mobile Wealth Hierarchy:** Re-orders the final wealth corpus card into an authoritative full-width hero on compact screens, preventing multi-digit currency truncation.
  12. **Fine-Grained Web Haptics Engine (`WebHapticEngine.ts`):** Delivers throttled mechanical click vibrations on slider milestones and 5-year investment horizons.
...
- **Clean Schema Markup (Zero Suppression):** `SoftwareApplication`, `FinancialProduct`, `FAQPage`, `BreadcrumbList`, `Article`, `WebPage`, and `HowTo` generated dynamically across all calculators and guides. Self-served `AggregateRating` schemas are strictly omitted to comply with Google Search Central guidelines and preserve rich snippet eligibility.
- **Static Pre-Calculated Scenario & Benchmark Tables (`scenario-benchmark-table.twig`):** Semantic HTML tables embedded on the homepage and across all calculator sub-pages, enabling Google rich answers and AI search engines (Gemini, Copilot, Perplexity) to scrape and quote verified numerical projections without client-side JavaScript execution.
...
| **Comprehensive Unit Test Suite & SEBI/AMFI Parity Architecture** | 836 unit and integration tests (13,751 assertions) with 100% pass rate and zero PHPUnit notices. Features dedicated compliance verification suites: (1) `SebiAmfiComplianceTest` verifying discrete monthly simulations against closed-form Annuity-Due formulas, Step-Up geometric progressions, SWP capital exhaustion zero-floors, Year 0 singularities, zero-rate linear fast paths, and Budget 2024 Section 112A LTCG tax accounting identities down to the paisa; (2) `SpecializedEnginesAlignmentTest` locking cross-runtime PHP/TS parity for Compound Interest, CAGR, EMI, FD, PPF, Inflation, and Safe SWP solvers; (3) `MathEngineAlignmentTest` enforcing paisa-precision tolerance across multi-decade vectors; (4) `EditorialReadingErgonomicsTest` locking reading dock, mobile TOC bottom sheets, table sticky columns, dynamic amount hydration, decoupled interactive wealth projection sandbox CTAs, and strict article bottom hierarchy sequencing; and (5) rigorous PHPUnit 13 test hygiene utilizing typed `createStub()` test doubles and purging redundant modal/styling assertions |
...
| **Editorial & Long-Form Reading Ergonomics** | Context-aware reading experience across `/resource/*` routes featuring: (1) fixed 3px GPU-accelerated reading progress bar (`#reading-progress-bar`) pinned directly beneath the navbar; (2) Mobile Ergonomic Reading Dock (`#mobile-reading-dock`) with live percentage telemetry (`#reading-progress-percent`), 1-tap "⚡ Calculate" jump anchor, native Web Share API dispatcher (`#mobile-reading-share-btn`), and safe-area inset bottom padding; (3) Mobile Table of Contents Native Bottom Sheet (`<dialog id="mobile-toc-sheet">`) with tactile drag handle (`.modal-drag-handle`), touch-target navigation items, and bidirectional `IntersectionObserver` scroll-spy synchronization across desktop (`#toc-list`) and mobile (`#mobile-toc-list`); (4) Responsive compact breadcrumbs with 1-tap parent back button on mobile screens (`sm:hidden`); (5) Frozen first-column sticky elevation (`.table-sticky-col-0`) and horizontal scroll boundary affordance masks (`.table-scroll-mask-end`) on editorial tables; (6) SSR default fallback text with client-side hydration (`hydrateDynamicAmounts()`) across all financial `.dynamic-amount` spans; (7) High-CTR pure light-mode Interactive Wealth Projection Sandbox CTA (`interactive-cta-banner.twig`) featuring tangible micro-sandbox previews, category-adaptive milestones (`growth`, `retirement`, `comparison`), verified institutional SEBI/AMFI trust badges, and kinetic action dispatchers; and (8) Optimal Bottom Hierarchy Sequencing (Article Body $\to$ Interactive Sandbox CTA $\to$ Author Bio $\to$ Continue Reading) maximizing reader conversion at peak motivation |
| **Mobile Viewport Ergonomics** | Context-scoped `#mobile-action-dock` featuring a Dual-Deck command capsule with real-time glance telemetry (`#dock-glance-corpus`), tap-to-scroll summary navigation, and safe-area inset bottom clamping (`pb-[max(0.625rem,env(safe-area-inset-bottom))]`); iOS 300ms double-tap zoom suppression via CSS `touch-action: manipulation` across all range sliders and stepper buttons; pure light-mode floating value bubble (`.slider-floating-tooltip` clamped 42px above range thumb in `SliderManager.ts` and `input.css`) eliminating finger contact patch occlusion; virtual keyboard accessory dock (`#keyboard-docked-preview`) with live corpus preview, next-key binding (`Enter` $\to$ `this.navigateInput(1)`), `pointerdown` blur prevention, auto-selection (`input.select()`), and touch-target expanded tactile SVG navigation chevrons; bidirectional inspection HUD elevation parity across direct canvas scrubbing and `#mobile-chart-scrubber` (`ChartScrubbingController.ts`); velocity-aware scroll-direction auto-collapsing of `#floating-discovery-hud` (`FloatingHudController.ts`); native bottom sheet action drawer (`#mobile-actions-sheet`) with strict `scrollTop <= 0` scroll-boundary gating (`BottomSheetGestureController.ts`); tactile swipeable modals with `.modal-drag-handle` visual grab pills; progressive `<details id="tax-statutory-steps-disclosure">` accordion in Section 112A Tax Waterfall modal eliminating compact-screen scroll collisions; edge-to-edge Horizontal Snap Carousel for Strategy Blueprints (`snap-x snap-mandatory`) reclaiming >320px vertical viewport dead-space; pure light-mode sticky first column elevation (`.table-sticky-col-0` with `backdrop-filter: blur(8px)` and right-facing drop shadows) and horizontal scroll boundary affordance masks (`.table-scroll-mask-end`) across Yearly Cashflow Ledger, Historical SIP Returns, and Investment Comparison tables; single-row touch-scrollable benchmark tracks; progressive stepper acceleration on long-press hold (`StepperController.ts`); milestone boundary haptic pulses (`WebHapticEngine.ts`); and direction-aware `#mobile-scroll-top-fab` elevated (`bottom-20`) to prevent occlusion of mobile docks |
...
| **SEBI & AMFI Regulatory Defaults & Bounds Alignment** | Complete regulatory synchronization of defaults and limits across all calculators: SIP default 12% (AMFI 15Y long-term Nifty 50 TRI benchmark), SWP default 8% (Conservative Hybrid benchmark) with 5% escalation (RBI CPI target band), LTCG taxation locked to Finance (No. 2) Act 2024 §112A (₹1.25L exemption, 12.5% rate). Lumpsum slider bounds harmonized to ₹25L accommodating all quick presets, and rate preset chips mapped to official AMFI risk-return asset categories (7% Debt, 9% Hybrid, 12% Large Cap, 14% Mid Cap, 18% Small Cap). |
