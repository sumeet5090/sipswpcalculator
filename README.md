...
- **Mathematical Solver Parity & Precision Hardening:** `InvestmentCalculator.php` and `MathEngine.ts` maintain 100% full method and algorithmic parity across all core compounding, delay cost (`calculateDelayCost`), inflation discounting (`calculateInflationDiscount`), required SIP (`calculateRequiredSip`), and SWP starting corpus (`calculateRequiredStartingCorpusForSwp`) reverse solvers. Upper bounds for SWP solvers dynamically compute cumulative escalated withdrawals to guarantee convergence even under hyper-escalating (10%–20% step-up) withdrawal regimes. SWR retirement feasibility benchmarks against starting retirement capital, adhering to Bengen's 4% rule.
...
| **Comprehensive Unit Test Suite** | 371 unit and integration tests (6,165 assertions) across Container, ActionDispatcher, Env, SiteConfig, DatabaseMigrator, FileRateLimitStorage, AdminAuthService, AdminDashboardPresenter, ShowAdminDashboardAction, CurrencyHelper, SessionManager, AppTwigExtension, ViteHelper, ServiceProviders, Middleware, Repositories, Security, Extreme Compounding, and Cross-Runtime Parity |
| **Mathematical Precision Guardrails** | Full encyclopedia of 165 mathematical engine, simulation, financial precision hazards, and parity constraints codified in `.agents/skills/mathematical-precision-guardrails/SKILL.md` |
