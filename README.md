...
- **PSR-11 Container & Strict DI:** `Core\Container` implements `Psr\Container\ContainerInterface` with `has()` and PSR-11 exception primitives (`NotFoundException`, `ContainerException`). Static database fallbacks (`DatabaseManager::getConnection()`) are replaced with explicit `\PDO` constructor injection across `InsightRepository`, `AnonymizedInsightLogger`, and `AdminController`.
