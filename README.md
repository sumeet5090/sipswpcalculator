...
| **Comprehensive Unit Test Suite & SEBI/AMFI Parity Architecture** | 834 unit and integration tests (13,738 assertions) with 100% pass rate and zero PHPUnit notices. Features dedicated compliance verification suites: (1) `SebiAmfiComplianceTest` verifying discrete monthly simulations against closed-form Annuity-Due formulas, Step-Up geometric progressions, SWP capital exhaustion zero-floors, Year 0 singularities, zero-rate linear fast paths, and Budget 2024 Section 112A LTCG tax accounting identities down to the paisa; (2) `SpecializedEnginesAlignmentTest` locking cross-runtime PHP/TS parity for Compound Interest, CAGR, EMI, FD, PPF, Inflation, and Safe SWP solvers; (3) `MathEngineAlignmentTest` enforcing paisa-precision tolerance across multi-decade vectors; (4) `EditorialReadingErgonomicsTest` locking reading dock, mobile TOC bottom sheets, table sticky columns, and dynamic amount hydration; and (5) rigorous PHPUnit 13 test hygiene utilizing typed `createStub()` test doubles and purging redundant modal/styling assertions |
