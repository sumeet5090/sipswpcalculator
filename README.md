...
| **Mobile Viewport Ergonomics** | Context-scoped `#mobile-action-dock` featuring a Dual-Deck command capsule with real-time glance telemetry (`#dock-glance-corpus`) and tap-to-scroll summary navigation; native bottom sheet action drawer (`#mobile-actions-sheet`) consolidating all exports (CSV, Branded PDF, Social Card, Copy Link, Snapshot, §112A Tax, Embed, and AMFI Rules); edge-to-edge Horizontal Snap Carousel for Strategy Blueprints (`snap-x snap-mandatory`) reclaiming >320px vertical viewport dead-space; pure light-mode sticky first column elevation (`.table-sticky-col-0` with `backdrop-filter: blur(8px)` and right-facing drop shadows) across Yearly Cashflow Ledger, Historical SIP Returns, and Investment Comparison tables; single-row touch-scrollable benchmark tracks; iOS numeric keypad accessory bar (`#keyboard-docked-preview`) with live corpus preview and "Done" dismissal; directional slope-locked touch chart scrubbing (`ChartScrubbingController.ts`); progressive stepper acceleration on long-press hold (`StepperController.ts`); milestone boundary haptic pulses (`WebHapticEngine.ts`); and direction-aware `#mobile-scroll-top-fab` |
