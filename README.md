...
| **Mobile Viewport Ergonomics** | Context-scoped `#mobile-action-dock` featuring a Dual-Deck command capsule with real-time glance telemetry (`#dock-glance-corpus`), tap-to-scroll summary navigation, and safe-area inset bottom clamping (`pb-[max(0.625rem,env(safe-area-inset-bottom))]`); iOS 300ms double-tap zoom suppression via CSS `touch-action: manipulation` across all range sliders and stepper buttons; native bottom sheet action drawer (`#mobile-actions-sheet`) and tactile swipeable modals (`#tax-waterfall-modal`, `#qr-share-modal`) with `.modal-drag-handle` visual grab pills; edge-to-edge Horizontal Snap Carousel for Strategy Blueprints (`snap-x snap-mandatory`) reclaiming >320px vertical viewport dead-space; pure light-mode sticky first column elevation (`.table-sticky-col-0` with `backdrop-filter: blur(8px)` and right-facing drop shadows) and horizontal scroll boundary affordance masks (`.table-scroll-mask-end`) across Yearly Cashflow Ledger, Historical SIP Returns, and Investment Comparison tables; single-row touch-scrollable benchmark tracks; iOS numeric keypad accessory bar (`#keyboard-docked-preview`) with live corpus preview, auto-select on touch focus (`input.select()`), coordinated floating HUD suppression, and "Done" dismissal; directional slope-locked touch chart scrubbing with elevated telemetry HUD highlights (`ChartScrubbingController.ts`); progressive stepper acceleration on long-press hold (`StepperController.ts`); milestone boundary haptic pulses (`WebHapticEngine.ts`); and direction-aware `#mobile-scroll-top-fab` |
