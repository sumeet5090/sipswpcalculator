...
- **Mobile Touch Ergonomics & Viewport Resiliency Architecture (`KeyboardViewportController.ts`, `ChartScrubbingController.ts`, `MobileErgonomicDeckController.ts`, `WebHapticEngine.ts`, `base.twig`):** Establishes an uncompromising mobile-first experience across compact viewports (320px–430px) adhering to Apple HIG (44pt) and Material Design 3 (48dp):
  ...
  5. **Asymmetric Mobile Wealth Hierarchy:** Re-orders the final wealth corpus card into an authoritative full-width hero on compact screens, preventing multi-digit currency truncation.
  6. **Fine-Grained Web Haptics Engine (`WebHapticEngine.ts`):** Delivers throttled mechanical click vibrations on slider milestones and 5-year investment horizons.
