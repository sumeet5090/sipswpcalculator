...
- **Strict Constructor Dependency Injection:** Services (`ViewRenderer`, `ContentManager`, `SitemapController`) require mandatory constructor parameters without implicit default instantiations (`new ViteHelper()`, `new Parsedown()`), enforcing the Pit of Success across the dependency graph.
