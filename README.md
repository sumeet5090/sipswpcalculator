...
- **ViteHelper (`Core\ViteHelper`):** Converted from a fully-static class to an instance service. Receives `string $environment` via constructor. Registered as a singleton in the DI container.
- **StrategyFactory (`Core\Strategies\StrategyFactory`):** Converted from a static utility to an instance service. Receives `ConfigService` and injects it into every `CalculatorStrategyInterface` it creates. `BaseStrategy` now declares a `ConfigService` property via constructor, propagating it explicitly to `InvestmentInputs::fromRequest()` and `fromSwpRequest()`.
- **InvestmentInputs (`Core\InvestmentInputs`):** `fromRequest()` and `fromSwpRequest()` now **require** a `ConfigService` instance — the implicit `$config ?? new \Services\ConfigService()` fallback has been eliminated, enforcing the Pit of Success.
- **SiteConfig (`Core\SiteConfig`):** All `Env::get()` calls removed from the class. The constructor receives `string $baseUrl` directly from the DI container binding.
- **AdminAuthService (`Core\AdminAuthService`):** The admin password is now injected via the constructor rather than read from environment variables inside the `login()` method.
