...
**Action:** Do not use PHP/Twig (`{% for %}`) to iterate and build HTML for calculator output arrays (like tables or charts). PHP must only serve empty `<tbody id="... ">` skeletons. TypeScript is solely responsible for rendering and updating dynamic DOM components via `CalculatorApp.ts`, `ResultsController.ts`, and `SummaryMetricsController.ts`.

### 2. Zero-Latency Calculation (No AJAX)
**Action:** Never propose or implement AJAX/API endpoints for financial math. All math must execute instantly in the browser via `assets/js/calculators/MathEngine.ts` and `assets/js/calculators/engines/*.ts`. 
If you modify formulas, you must verify parity against `tests/Unit/SebiAmfiComplianceTest.php`, `tests/Unit/SpecializedEnginesAlignmentTest.php`, and `tests/parity_check.php`.
...
**Action:** Do not use CSS to hide massive forms. Build specific backend Twig components (`sip-fields.twig`, `swp-fields.twig`, specialized fields) and serve only what the user requested. `CalculatorApp.ts` and specialized controllers must be written defensively to default missing DOM elements without throwing errors.

### 5. SEO & Structured Data (Mandatory Dual-Schema Rule)
**Action:** Every calculator page must inject both a `SoftwareApplication` schema and an `FAQPage` schema (via `SchemaHelper.php` and `HomeSchemaBuilder.php`). Titles must strictly remain between **10 and 65 characters** (enforced by `SeoMetadataValidatorTest.php`). Do not use generic names; match the schema name precisely to the URL intent.
...
- **Environment Precedence:** Always use `Env::get($key, $default)` rather than direct `$_ENV` access. The resolution hierarchy is `$_ENV` > `$_SERVER` > `getenv()` > `$default`. Env calls reside exclusively at the DI boundary.
...
> **Further Reading:** For a deep dive into these rules, consult the root `README.md` and Architectural Decision Records in `docs/adr/`.
